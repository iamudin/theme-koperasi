...
<section class="relative text-white">
  ...
  <div class="relative z-10 max-w-7xl mx-auto px-4 py-16 md:py-20">
    <div class="grid lg:grid-cols-2 gap-10 items-center">
      <div>
        <div
          class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs text-slate-200">
        ...
        <h1 class="mt-6 text-3xl md:text-5xl font-black leading-tight">
          {{ get_option('nama_organisasi') ?? 'Nama Instansi / Organisasi' }}
        </h1>
        <div class="mt-4 text-slate-200 leading-relaxed"> <i class="fa fa-map-marker-alt"></i>
          {{ get_option('alamat') ?? 'Alamat Organisasi / Instansi / Unit Kerja / Tim / Grup' }}</div>
        <div class="mt-8 flex flex-wrap gap-3">
          <a href="#layanan"
            class="inline-flex items-center gap-2 rounded-xl bg-emerald-400 px-5 py-3 text-sm font-bold text-slate-950 hover:bg-emerald-300">
            <i class="fa fa-bolt"></i>
            <span>Layanan Utama</span>
          </a>
          <a href="#tupoksi"
            class="inline-flex items-center gap-2 rounded-xl bg-white/10 px-5 py-3 text-sm font-semibold text-white hover:bg-white/15">
  ...
  <div class="rounded-3xl overflow-hidden shadow border border-white/10 dark:border-slate-800 relative">
    <div id="homeBannerSlider" class="relative aspect-[16/7]">
      @if($bannersHome && count($bannersHome) > 0)
        @foreach($bannersHome as $i => $banner)
          <div
            class="banner-slide bg-gradient-to-r from-emerald-600 via-emerald-500 to-slate-900 absolute inset-0 opacity-0 transition-opacity duration-500 {{ $i === 0 ? 'opacity-100' : '' }}">
    ...
    @if($bannersHome && count($bannersHome) > 1)
      <button id="prevBanner"
        class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/20 hover:bg-white flex items-center justify-center text-slate-800 shadow-lg transition hover:scale-110">
        <i class="fa fa-chevron-left"></i>
      </button>
      <button id="nextBanner"
        class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/20 hover:bg-white flex items-center justify-center text-slate-800 shadow-lg transition hover:scale-110">
      ...
      <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
        @foreach($bannersHome as $i => $banner)
          <button
            class="banner-dot w-2 h-2 rounded-full transition {{ $i === 0 ? 'bg-white' : 'bg-white/50' }} hover:bg-white"
            data-index="{{ $i }}"></button>
        @endforeach
      </div>
    @endif
...
<section id="tupoksi" class="max-w-7xl mx-auto px-4 py-12">
  <div class="grid lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 rounded-3xl bg-white p-6 shadow dark:bg-slate-900">
      <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-100"> <i class="fa fa-hand"></i> Sambutan Pimpinan
      </h2>
      <div class="mt-4 text-slate-700 leading-relaxed dark:text-slate-200">
        {!! str($sambutan->content ?? 'Selamat datang di website resmi kami')->limit(1000) . ' <a class="font-bold" href="' . url($sambutan->url ?? '/') . '">[Selengkapnya]</a>' ?? 'Selamat datang di website resmi kami' !!}
      </div>
      <div class="mt-6 flex items-center gap-4">
        ...
        <div>
          <div class="text-sm text-slate-500 dark:text-slate-400">{{ $sambutan->field?->jabatan ?? 'Kepala Dinas' }}
          </div>
    ...
    <div class="rounded-3xl bg-white p-6 shadow dark:bg-slate-900">
      ...
      <div class="mt-4 space-y-3">
        <a href="#layanan"
          class="block rounded-2xl border border-slate-200 p-4 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800">
...
<section id="layanan" class="max-w-7xl mx-auto px-4 py-12">
  <div class="flex flex-col lg:grid lg:grid-cols-[3fr_7fr] gap-8 items-start">
    ...
    <div class="w-full">
      ...
      <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 lg:gap-6">
...
<section id="berita" class="max-w-7xl mx-auto px-4 py-12">
  ...
  <div class="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
    @forelse(query()->index_limit('berita', 3) as $row)
      <a href="{{ $row->link }}"
        class="group rounded-3xl overflow-hidden bg-white shadow hover:shadow-lg transition dark:bg-slate-900">
        <div class="aspect-[16/9] bg-slate-100 dark:bg-slate-800">
          <img src="{{ $row->thumbnail }}" alt="{{ $row->title }}" class="h-full w-full object-cover">
        </div>
        <div class="p-5">
          <div class="text-xs text-slate-500 dark:text-slate-400"> <i class="fa fa-clock"></i>
            {{ $row->created_at?->diffForHumans() ?? '' }} oleh {{ $row->user->name }}</div>
          <div class="mt-1 font-bold text-slate-900 group-hover:text-emerald-700 line-clamp-2 dark:text-slate-100">
            {{ $row->title }}
          </div>
    ...
    @empty
      <div class="rounded-3xl bg-white p-6 shadow text-slate-600 dark:bg-slate-900 dark:text-slate-300">Belum ada berita.
      </div>
    @endforelse
  </div>
</section>
...
<section id="galeri" class="max-w-7xl mx-auto px-4 py-12">
  ...
  <div class="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
    @forelse(query()->index_limit('galeri', 3) as $row)
      <a href="{{ $row->link }}" class="group rounded-3xl overflow-hidden shadow hover:shadow-lg transition block">
        <div class="relative aspect-[4/3]">
          <img src="{{ $row->thumbnail }}" alt="{{ $row->title }}"
            class="h-full w-full object-cover group-hover:scale-105 transition duration-300 bg-slate-100 dark:bg-slate-800">
          <div class="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent"></div>
          @if(!empty($row->data_field['link_video']))
            <div
              class="absolute top-4 left-4 w-12 h-12 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
    ...
    @empty
      <div class="rounded-3xl bg-white p-6 shadow text-slate-600 dark:bg-slate-900 dark:text-slate-300">Belum ada galeri.
      </div>
    @endforelse
  </div>
</section>
...
<section id="kepegawaian" class="max-w-7xl mx-auto px-4 py-12">
  ...
  <div id="kepegawaianSlider" class="mt-6 flex overflow-x-auto gap-4 lg:gap-6 snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden -mx-4 px-4 lg:mx-0 lg:px-0 py-4">
    @forelse(query()->index_sort('kepegawaian') as $row)
...
<div id="searchModal"
  ...
  <div
    class="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
    <div class="p-6">
      <div class="flex items-center justify-between mb-6">
        ...
        <button type="button" onclick="closeSearchModal()"
          class="h-10 w-10 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 transition">
          <i class="fa fa-times"></i>
        </button>
      </div>
      <form action="{{ url('search') }}" method="post" class="space-y-4">
        <div class="relative">
          <input type="text" id="searchInput" name="keyword" placeholder="Ketik pencarian Anda..."
            class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-5 py-4 text-slate-900 placeholder:text-slate-500 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder:text-slate-400 transition">
          <i class="fa fa-search absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400"></i>
        </div>
        <div class="flex gap-3 justify-end">
          <button type="button" onclick="closeSearchModal()"
            class="px-6 py-3 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 transition">Batal</button>
          <button type="submit"
            class="px-6 py-3 rounded-xl bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition">Cari</button>
        </div>
      </form>
    </div>
  </div>
